...
import {
  ...
  reserveEngineById,
  toEngineViews,
} from "@app/engine/engine-pool.helpers.js";
 ...
 *
 * @param config - (PlatformConfig) Runtime platform configuration.
 * @param stateRef - (Ref.Ref<ReadonlyMap<string, EngineRuntimeState>>) Scheduler state.
...
const listEngines = (
  ...
  Ref.get(stateRef).pipe(
    Effect.map(
      (states: ReadonlyMap<string, EngineRuntimeState>): readonly EngineView[] =>
        toEngineViews(config, states),
    ),
  );
...
const markEngineFailure = (
  ...
  Clock.currentTimeMillis.pipe(
    Effect.flatMap((nowEpochMs: number): Effect.Effect<void> =>
      Ref.update(
        stateRef,
        (states: ReadonlyMap<string, EngineRuntimeState>): ReadonlyMap<
          string,
          EngineRuntimeState
  ...
  Clock.currentTimeMillis.pipe(
    Effect.flatMap(
      (nowEpochMs: number): Effect.Effect<Option.Option<EngineReservation>> =>
        Ref.modify(
          stateRef,
          (states: ReadonlyMap<string, EngineRuntimeState>): readonly [
 ...
 * @param stateRef - (Ref.Ref<ReadonlyMap<string, EngineRuntimeState>>) State reference.
...
const createEnginePool = (
  ...
  list: (): Effect.Effect<readonly EngineView[]> => listEngines(config, stateRef),
  ...
  release: (engineId: string): Effect.Effect<void> => releaseEngine(stateRef, engineId),
